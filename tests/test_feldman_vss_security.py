...
from .test_conftest import DEFAULT_NUM_SHARES, DEFAULT_THRESHOLD, MockField, generate_poly_and_shares
...
def test_detect_collusion_no_pattern(default_vss: FeldmanVSS):
    ...
    invalid_shares_detected = {4: [1], 5: [2], 6: [3]}
    echo_consistency = {}
    potential_colluders = default_vss._enhanced_collusion_detection(invalid_shares_detected, party_ids, echo_consistency)
...
def test_quantum_verification_no_dlog(default_vss: FeldmanVSS, test_coeffs: list[mpz], test_shares: ShareDict):
    ...
    with (
        patch.object(default_vss.group, "exp", side_effect=AssertionError("exp should not be called in hash verification")) as mock_exp,
        patch.object(
            default_vss.group, "secure_exp", side_effect=AssertionError("secure_exp should not be called in hash verification")
        ) as mock_secure_exp,
...
def test_side_channel_vulnerability_acknowledgement(default_vss: FeldmanVSS, test_coeffs_shares):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        if not timing_warnings:
            test_logger.warning(
                "Matrix operations (_secure_matrix_solve, _find_secure_pivot) "
