...
class TestSerialization:
    ...
    def test_commitment_proof_serialization_roundtrip(self, default_vss: FeldmanVSS, test_coeffs: list[mpz]):
        ...
        assert all(deser_proof["blinding_commitments"][i] == proof["blinding_commitments"][i] for i in range(len(proof["blinding_commitments"])))
        # Compare randomizers
    ...
    def test_deserialization_different_hash(self, vss_blake3: FeldmanVSS, vss_sha3: FeldmanVSS, commitments_blake3: CommitmentList, commitments_sha3: CommitmentList):
...
class TestPedersenIntegration:
    ...
    def test_create_dual_commitment_proof(self, setup_integration):
        ...
        proof = create_dual_commitment_proof(
            feldman_vss, pedersen_vss, coeffs, feldman_commits, pedersen_commits
        )
    ...
    def test_verify_dual_commitments_valid(self, setup_integration):
        ...
        is_valid = verify_dual_commitments(
            feldman_vss, pedersen_vss, feldman_commits, pedersen_commits, proof
        )
    ...
    def test_verify_dual_commitments_invalid_proof(self, setup_integration):
        ...
        is_valid = verify_dual_commitments(
            feldman_vss, pedersen_vss, feldman_commits, pedersen_commits, tampered_proof
        )
        ...
        is_valid_chal = verify_dual_commitments(
            feldman_vss, pedersen_vss, feldman_commits, pedersen_commits, tampered_proof_chal
        )
    ...
    def test_verify_dual_commitments_mismatched_commits(self, setup_integration):
        ...
        is_valid = verify_dual_commitments(
            feldman_vss, pedersen_vss, feldman_commits_alt, pedersen_commits_alt, proof
        )
        assert is_valid is False
    ...
    def test_verify_dual_commitments_length_errors(self, setup_integration):
        ...
        with pytest.raises(ValueError, match="same length"):
            verify_dual_commitments(
                feldman_vss, pedersen_vss, feldman_commits[:-1], pedersen_commits, proof
            )
        with pytest.raises(ValueError, match="same length"):
            verify_dual_commitments(
                feldman_vss, pedersen_vss, feldman_commits, pedersen_commits[:-1], proof
            )
        ...
        with pytest.raises(ValueError, match="Number of responses"):
            verify_dual_commitments(
                feldman_vss, pedersen_vss, feldman_commits, pedersen_commits, short_proof
            )
        ...
        if "response_randomizers" in short_proof_rand:
            ...
            is_valid = verify_dual_commitments(
                feldman_vss, pedersen_vss, feldman_commits, pedersen_commits, short_proof_rand
            )
            assert is_valid is False
...
class TestShareRefreshing:
    ...
    def test_refresh_shares_preserves_secret(self, default_vss: FeldmanVSS, test_coeffs: list[mpz], test_shares: ShareDict, test_commitments: CommitmentList, mock_shamir: MockShamirSecretSharing):
        ...
        new_shares, new_commitments, verification_data = default_vss.refresh_shares(
            test_shares, t, n, test_commitments, participant_ids
        )
    ...
    def test_refresh_shares_threshold_cases(self, t: int, n: int, default_vss: FeldmanVSS, mock_field_fast: MockField, mock_shamir: MockShamirSecretSharing):
        ...
        new_shares, new_commitments, _ = default_vss.refresh_shares(
            shares, t, n, commitments, participant_ids
        )
    ...
    def test_refresh_shares_insufficient_input_shares(self, default_vss: FeldmanVSS, test_shares: ShareDict, test_commitments: CommitmentList):
        ...
        with pytest.raises(ParameterError, match=f"Need at least {t} shares"):
            default_vss.refresh_shares(
                insufficient_shares, t, n, test_commitments, participant_ids
            )
