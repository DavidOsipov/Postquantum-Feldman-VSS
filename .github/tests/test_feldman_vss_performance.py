...
class TestPerformance:
    ...
    def test_perf_1_create_commitments_large(self, large_test_data: Optional[dict[str, Any]]):
        ...
        avg_time, min_t, max_t, std_dev, _ = _benchmark(
            vss.create_commitments, coeffs, iterations=self.ITERATIONS
        )
        test_logger.info(f"[Benchmark] Create Commitments (t={t}): Avg={avg_time:.4f}s (Min={min_t:.4f}, Max={max_t:.4f}, StdDev={std_dev:.4f})")
    ...
    def test_perf_2_batch_verify_large(self, large_test_data: Optional[dict[str, Any]]):
        ...
        avg_time, min_t, max_t, std_dev, results = _benchmark(
            vss.batch_verify_shares, share_list, commitments, iterations=self.ITERATIONS
        )
        test_logger.info(f"[Benchmark] Batch Verify (n={n}, t={t}): Avg={avg_time:.4f}s (Min={min_t:.4f}, Max={max_t:.4f}, StdDev={std_dev:.4f})")
        assert avg_time >= 0
    ...
    def test_perf_3_sequential_verification(self, large_test_data: Optional[dict[str, Any]]):
        ...
        test_logger.info(f"[Benchmark] Sequential Verify ({num_verify} shares, t={t}): AvgTotal={avg_time:.4f}s, AvgPerShare={avg_time_per_share:.6f}s")
        assert avg_time >= 0
    ...
    def test_perf_4_zkp_creation_verification_large(self, large_test_data: Optional[dict[str, Any]]):
        ...
        test_logger.info(f"[Benchmark] ZKP Create (t={t}): Avg={avg_time_create:.4f}s (Min={min_t_c:.4f}, Max={max_t_c:.4f}, StdDev={std_dev_c:.4f})")
        ...
        test_logger.info(f"[Benchmark] ZKP Verify (t={t}): Avg={avg_time_verify:.4f}s (Min={min_t_v:.4f}, Max={max_t_v:.4f}, StdDev={std_dev_v:.4f})")
    ...
    def test_perf_5_refresh_shares_large(self, large_test_data: Optional[dict[str, Any]]):
        ...
        avg_time, min_t, max_t, std_dev, results = _benchmark(
             vss.refresh_shares,
             shares, t, n, commitments, participant_ids, # Pass participant_ids explicitly
             iterations=current_iterations
        )
        test_logger.info(f"[Benchmark] Refresh Shares (n={n}, t={t}): Avg={avg_time:.4f}s (Min={min_t:.4f}, Max={max_t:.4f}, StdDev={std_dev:.4f})")
        assert avg_time >= 0
